document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyTheme(theme, isInitialLoad = false) {
        ...
        if (theme === 'stars' && typeof particlesJS !== 'undefined') {
            ...
            if (particlesElement && particlesElement.childElementCount === 0) {
    ...
    function renderNav() {
        ...
        if (categories.length === 0) {
            mainNav.innerHTML = `<span style="color: var(--text-secondary);">เริ่มโดยการเพิ่มหมวดหมู่</span>`;
        }
    ...
    function renderCategoryView() {
        ...
        if (Object.keys(categoryData).length === 0) {
            grid.innerHTML = `<p>ยังไม่มีกราฟย่อยในหมวดนี้</p>`;
        } else {
            Object.entries(categoryData).forEach(([name, data]) => {
                const card = createSubmetricCard(name, data);
                grid.appendChild(card);
                const canvas = card.querySelector('canvas');
                if (canvas && data.entries && data.entries.length > 0) {
                    renderSubmetricChart(canvas, name, data);
                }
            });
        }
    ...
    document.getElementById('confirm-submetric').addEventListener('click', () => {
        ...
        if (!newName) { alert('กรุณาใส่ชื่อหมวดหมู่'); return; }
